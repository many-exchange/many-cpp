...
struct TransactionReponse {
  ...
  TransactionReponse(uint64_t blockTime, struct Meta meta, uint64_t slot, struct Transaction transaction)
    : blockTime(blockTime), meta(meta), slot(slot), transaction(transaction) {}

  friend std::ostream& operator<<(std::ostream& os, const TransactionReponse& transaction) {
...
public:
  ...
  */
  ...
  TransactionReponse getTransaction(std::string transactionSignature) {
    ...
    std::vector<TransactionReponse::Transaction::Message::Instruction> instructions;
    for (auto instruction : transaction["message"]["instructions"]) {
      std::vector<uint8_t> accounts;
      for (auto account : instruction["accounts"]) {
        accounts.push_back(account.get<uint64_t>());
      }
      std::string data = instruction["data"].get<std::string>();
      instructions.push_back(TransactionReponse::Transaction::Message::Instruction {
        accounts,
        data,
        instruction["programIdIndex"].get<uint8_t>()
      });
    }

    std::vector<uint64_t> preBalances;
    for (auto preBalance : result["meta"]["preBalances"]) {
      preBalances.push_back(preBalance.get<uint64_t>());
    }
    std::vector<uint64_t> postBalances;
    for (auto postBalance : result["meta"]["postBalances"]) {
      postBalances.push_back(postBalance.get<uint64_t>());
    }

    std::vector<std::string> logMessages;
    for (auto logMessage : result["meta"]["logMessages"]) {
      logMessages.push_back(logMessage.get<std::string>());
    }

    std::vector<TransactionReponse::Meta::TransactionReward> rewards;
    for (auto reward : result["meta"]["rewards"]) {
      rewards.push_back(TransactionReponse::Meta::TransactionReward {
    ...
    std::vector<PublicKey> writable;
    for (auto writableAccount : result["meta"]["loadedAddresses"]["writable"]) {
      writable.push_back(PublicKey(writableAccount.get<std::string>()));
    }
    std::vector<PublicKey> readOnly;
    for (auto readOnlyAccount : result["meta"]["loadedAddresses"]["readOnly"]) {
      readOnly.push_back(PublicKey(readOnlyAccount.get<std::string>()));
    }

    return {
      result["blockTime"].get<uint64_t>(),
      {
        result["meta"]["err"].is_null() ? "" : result["meta"]["err"].get<std::string>(),
        result["meta"]["fee"].get<uint64_t>(),
        //TODO this should be an array of instruction result["meta"]["innerInstructions"].get<std::string>(),
        {
          writable,
          readOnly
        },
        ...
        //TODO this should be optional
        //{
          //PublicKey(result["meta"]["returnData"]["programId"].get<std::string>()),
          //result["meta"]["returnData"]["data"].get<std::string>(),
        //},
      },
      result["slot"].get<uint64_t>(),
      {
        {
          accountKeys,
          {
            transaction["message"]["header"]["numReadonlySignedAccounts"].get<uint8_t>(),
            transaction["message"]["header"]["numReadonlyUnsignedAccounts"].get<uint8_t>(),
            transaction["message"]["header"]["numRequiredSignatures"].get<uint8_t>(),
          },
          instructions,
          transaction["message"]["recentBlockhash"].get<std::string>()
        },
        signatures,
      },
    };
